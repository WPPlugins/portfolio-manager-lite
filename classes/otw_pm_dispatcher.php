...
class OTWPMLDispatcher {
  ...
  public function generateTemplate ( $pm_options = null, $pm_results = null, $templateMediaOptions = null, $ajax = false, $ajaxPage = null ) {
    ...
    if( empty( $pm_options ) || empty( $pm_results ) ) {
      throw new Exception(_e('There was an error in OTWPMLDispatcher: $results or $options is missing', OTW_PML_TRANSLATION), 1);
    }
    ...
    if( !has_filter( 'the_content', 'wpautop' ) && !$ajax && !$pm_options['widget']) {
  ...
  private function buildInterfacePortfolioItems ( $post ) {
    if( empty( $post ) ) {
      throw new Exception(_e('There was an error in OTWPMLDispatcher -> buildInterfacePortfolioItems ', OTW_PML_TRANSLATION), 1);
    }
  ...
  private function loadTemplate ( $templateName, $otw_pm_posts ) {
    ob_start();
    if( file_exists( OTW_PML_SERVER_PATH . DS . 'skeleton' . DS . $templateName . '.php' ) ){
